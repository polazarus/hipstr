...
impl<T: Copy, L: Layout<T>> InlineVec<T, L> {
    ...
    /// let mut v = CopyInlineVec::with_capacity(5);
    ///
    ...
    /// // Creates the vector.
    /// let size = 4;
    /// let mut x: CopyInlineVec<i32> = CopyInlineVec::with_capacity(size);
    ...
    /// # use hipvec::inline::CopyInlineVec;
    ///
    /// let mut x: CopyInlineVec<i32> = CopyInlineVec::new();
    ...
    /// ```
    /// # use hipvec::copy_inline_vec;
    /// let mut vec = copy_inline_vec![0_u8, 1];
    ...
    /// ```
    /// # use hipvec::copy_inline_vec;
    /// let mut vec = copy_inline_vec![1_u8, 2, 3, 4];
    ...
    /// ```
    /// # use hipvec::copy_inline_vec;
    /// let mut vec = copy_inline_vec![1_u8];
    ...
    /// ```
    /// # use hipvec::copy_inline_vec;
    /// let mut v = copy_inline_vec!['a', 'b', 'c'];
    /// let w = v.split_off(1);
    ...
    #[inline]
    #[track_caller]
    pub const fn resize(&mut self, new_len: usize, value: T)
    where
        T: Clone,
    {
...
impl<T: Copy, L: Layout<T>> ops::Deref for InlineVec<T, L> {
    type Target = [T];
    fn deref(&self) -> &[T] {
        self.as_slice()
    }
...
impl<T: Copy, L: Layout<T>> ops::DerefMut for InlineVec<T, L> {
